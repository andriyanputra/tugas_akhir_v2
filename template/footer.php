...
<script type="text/javascript">
    $(function() {
        ...
        var $box = $(this).closest('.infobox');             var barColor = !$box.hasClass('infobox-dark') ? $box.css('color') : '#FFF';             $(this).sparkline('html', {tagValuesAttribute: 'data-values', type: 'bar', barColor: barColor, chartRangeMin: $(this).data('min') || 0});
        });
        ...
        $.plot("#sales-charts", [
            {label: "Domains", data: d1},             {label: "Hosting", data: d2},
        ...
        var agent = navigator.userAgent.toLowerCase();
                if ("ontouchstart" in document && /applewebkit/.test(agent) && /android/.test(agent))             $('#tasks').on('touchstart', function(e) {
        ...
        $(ui.item).css('z-index', 'auto');
                ...
                $('#tasks').disableSelection();         $('#tasks input:checkbox').removeAttr('checked').on('click', function() {
                if (this.checked)
...
<?php date_default_timezone_set('Asia/Jakarta'); ?>
    ...
    var Diff = serverTime.getTime() - clientTime.getTime();
            ...
            function displayServerTime() {         //buat object date berdasarkan waktu di client
            var clientTime = new Date();
        ...
        //tampilkan jam:menit:detik dengan menambahkan angka 0 jika angkanya cuma satu digit (0-9)         document.getElementById("clock").innerHTML = (sh.length == 1 ? "0" + sh : sh) + ":" + (sm.length == 1 ? "0" + sm : sm) + ":" + (ss.length == 1 ? "0" + ss : ss);
    }
</script>
...
<script type="text/javascript">
    $(function() {
        ...
        $.fn.editableform.loading = "<div class='editableform-loading'><i class='light-blue icon-2x icon-spinner icon-spin'></i></div>";
            ...
            //editables 
        $('#username').editable({             type: 'text',
            name: 'username'
        });

        var countries = [];
        $.each({"CA": "Canada", "IN": "India", "NL": "Netherlands", "TR": "Turkey", "US": "United States"}, function(k, v) {             countries.push({id: k, text: v});
        }); 
        var cities = [];
            cities["CA"] = [];
        ...
        try {//ie8 throws some harmless exception, so let's catch it
            ...
            Image.prototype.appendChild = function(el) {
                ...
                value: null,
                    image: {
                     //specify ace file input plugin's options here                     btn_choose: 'Change Avatar',
                    droppable: true,
                    /**
        ...
        $('#avatar2').on('click', function() {
            ...
            file.ace_file_input({
                ...
                working = true;
                    ...
                    var thumb = file.next().find('img').data('thumb');                     if (thumb)
